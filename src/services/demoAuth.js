...
export async function demoForgotPassword(email) {
  ...
  return {
    message:
      'If an account exists with that email, reset instructions have been sent. (Demo: register a new account in this browser.)',
  }
}

export async function demoResetPassword() {
  throw new Error(
    'Password reset is not available in the GitHub Pages demo. Please register again.'
  )
}
